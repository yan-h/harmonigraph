...
export const EXAMPLES = [
  ...
  { id: 'released-e', name: 'Released low E', category: 'Memory',
    description: 'Release the low E immediately before high E. It remains remembered, but while the chain sounds only held notes count.',
    program: 'on C3 c\non G3 g\non D4 d\non A4 a\non E3 low\noff low\non E5 high', expectedNode: [4, 0, 0], expected: 'High E continues the fifth chain.' },
  { id: 'passing', name: 'A passing note over an old chord', category: 'Memory',
    description: 'C major is held three seconds, then D is struck and let go before A. A released note never outranks a held one, however long that has been held, so A is tuned to the sounding chord.',
    program: 'on C3 c\non E3 e\non G3 g\nwait 3\non D4 d\nwait 0.1\noff d\non A4 a', expectedNode: [-1, 1, 0], expected: 'A at 5/3, a just fourth above the held E; following the released D it would be 27/16.' },
  { id: 'precise-e', name: 'Intentional Pythagorean E', category: 'Precision', settings: { pitchFlexibility: 50 },
    seed: [...chain, seed('low', 'E3', [0, 1, 0])],
    description: 'Starting pitches and zero reference are seeded explicitly. Lower pitch flexibility allows precise input to choose a different E from the one sounding.',
    program: `on ${(4800 + 4 * AXES[0]).toFixed(9)}c high`, expectedNode: [4, 0, 0], expected: 'Precise input selects the Pythagorean E.' },
  { id: 'nominal-e', name: 'Same context · ordinary E', category: 'Precision', settings: { pitchFlexibility: 50 },
    seed: [...chain, seed('low', 'E3', [0, 1, 0])],
