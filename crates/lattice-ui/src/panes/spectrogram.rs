...
/// A flat spectrogram of `columns` drawn into `rect` — pitch up (the octave
/// zoom, low at the bottom), time across (start left, end right). Reuses the
/// same aggregation and coloring as the depth-mapped spectrogram so the Render
/// pane's bounce preview reads in the same scheme. `tex_slot` is a handle of its
/// own (slot 2) so it never clobbers the two live spectrograms.
pub(super) fn draw_flat_spectrogram(
    painter: &egui::Painter,
    rect: egui::Rect,
    cfg: &SpectrumConfig,
    frame: &FrameParams,
    ...
    let opacity = cfg.spectrogram_opacity.clamp(0.0, 1.0);
    if opacity <= 0.0 || columns.len() < 2 || rect.width() < 2.0 || rect.height() < 2.0 {
        return;
    }
    // The Spectrum tab's octave zoom, as a pitch range.
    let min_midi = lattice_core::notes::octave_start_midi(cfg.low_octave) as f32;
    let max_midi = lattice_core::notes::octave_start_midi(cfg.high_octave) as f32;
    let pspan = (max_midi - min_midi).max(1.0);
    let bin_semis = 1.0 / BINS_PER_SEMITONE as f32;
    let bins: Vec<Bin> = (0..SPECTRUM_BINS)
        .filter_map(|idx| {
            let midi = SPECTRUM_MIN_MIDI + (idx as f32 + 0.5) * bin_semis;
            let t = (midi - min_midi) / pspan;
            (t > -0.02 && t < 1.02).then_some(Bin { idx, midi, t })
        })
        .collect();
    if bins.len() < 2 {
        return;
    ...
    // One image column per output pixel, but never finer than the FFT arrives.
    let target_cols = rect.width().round().clamp(2.0, 4096.0) as usize;
    const MIN_BUCKET: f64 = 0.08;
    let bucket = (span / target_cols as f64).max(MIN_BUCKET);
    let bin_idx: Vec<usize> = bins.iter().map(|b| b.idx).collect();
    let (centers, mut power) = aggregate_rows(columns.iter(), &bin_idx, bucket);
    let w = centers.len();
    if w < 2 {
        return;
    }
    smooth_time(&mut power, w, bins.len(), cfg.spectrogram_smoothing);

    let pixels = fill_pixels(cfg, frame, w, &bins, &power);
    let image = egui::ColorImage::new([w, bins.len()], pixels);
    let opts = egui::TextureOptions::LINEAR;
    if let Some(handle) = tex_slot.as_mut() {
        handle.set(image, opts);
    } else {
        *tex_slot = Some(painter.ctx().load_texture("bounce-spectrogram", image, opts));
    }
    let Some(tex) = tex_slot.as_ref() else { return };

    // One quad over the strip: image row 0 (low pitch) at the bottom, time left
    // to right.
    let tint = Color32::from_white_alpha((opacity * 255.0) as u8);
    let vert = |x: f32, y: f32, u: f32, v: f32| egui::epaint::Vertex {
        pos: egui::pos2(x, y),
        uv: egui::pos2(u, v),
        color: tint,
    };
    let mut mesh = egui::Mesh::with_texture(tex.id());
    mesh.vertices.push(vert(rect.left(), rect.bottom(), 0.0, 0.0));
    mesh.vertices.push(vert(rect.right(), rect.bottom(), 1.0, 0.0));
    mesh.vertices.push(vert(rect.right(), rect.top(), 1.0, 1.0));
    mesh.vertices.push(vert(rect.left(), rect.top(), 0.0, 1.0));
    mesh.add_triangle(0, 1, 2);
    mesh.add_triangle(0, 2, 3);
    painter.add(egui::Shape::mesh(mesh));
}
