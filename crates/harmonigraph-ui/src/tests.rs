...
/// A pane dragged down to the size a fold would leave it at folds when the drag
/// is let go of — and because those two sizes are the same number, nothing moves
/// when it does: not the pane, not its neighbour, not the window.
///
/// Without it the dock has two states that look identical and behave differently:
/// a pane at its floor draws a tab bar's worth of nothing, keeps its share of the
/// window, and has none of a rail's name or arrow.
#[test]
fn a_pane_dragged_down_to_a_rails_width_folds_when_it_is_let_go_of() {
    let mut h = DockHarness::new();
    let mut state = SharedState::new(TextureFormat::Bgra8Unorm);
    h.settle(&mut state);
    ...
    // The separator between the lattice and the analyzer, dragged well past the
    // analyzer's floor — egui_dock's clamp stops it exactly there.
    let analyzer = pane_rect(&state, panes::Tab::Spectral);
    let at = egui::pos2(analyzer.left() - style.separator.width * 0.5, analyzer.center().y);
    h.frame(&mut state, vec![egui::Event::PointerMoved(at)]);
    h.frame(&mut state, vec![egui::Event::PointerMoved(at), press(at, true)]);
    let target = at + egui::vec2(analyzer.width() + 200.0, 0.0);
    h.frame(&mut state, vec![egui::Event::PointerMoved(target)]);
    h.frame(&mut state, vec![egui::Event::PointerMoved(target)]);

    let floored = pane_width(&state, panes::Tab::Spectral);
    assert!(
        (floored - rail).abs() < 1.0,
        "the drag floor should be a rail's width, and left the analyzer at {floored}",
    );
    assert!(!collapsed(&state, panes::Tab::Spectral), "and it is not folded until let go of");
    let before = (pane_width(&state, panes::Tab::Lattice), h.screen.width());

    h.frame(&mut state, vec![press(target, false)]);
    assert!(
        collapsed(&state, panes::Tab::Spectral),
        "letting go at the floor should have folded the analyzer",
    );
    let _ = h.settle_folds(&mut state);
    let after = (pane_width(&state, panes::Tab::Lattice), h.screen.width());
    assert!(
        (pane_width(&state, panes::Tab::Spectral) - rail).abs() < 1.0,
        "the rail it became is the width it already had",
    );
    assert!((after.0 - before.0).abs() < 1.0, "the lattice should not have moved");
    assert!((after.1 - before.1).abs() < 1.0, "and the window should not have resized");
}

/// A drag that ends anywhere else leaves the pane alone, or every resize would be
/// one slip away from folding something.
#[test]
fn a_drag_that_ends_short_of_the_floor_folds_nothing() {
    let mut h = DockHarness::new();
    let mut state = SharedState::new(TextureFormat::Bgra8Unorm);
    h.settle(&mut state);
    let separator = theme::dock_style(&egui::Style::default(), 1.0).separator.width;
    let analyzer = pane_rect(&state, panes::Tab::Spectral);
    let at = egui::pos2(analyzer.left() - separator * 0.5, analyzer.center().y);
    h.frame(&mut state, vec![egui::Event::PointerMoved(at)]);
    h.frame(&mut state, vec![egui::Event::PointerMoved(at), press(at, true)]);
    // Most of the way, and then back out again: the floor was touched, and the
    // drag did not end there.
    let far = at + egui::vec2(analyzer.width() + 200.0, 0.0);
    h.frame(&mut state, vec![egui::Event::PointerMoved(far)]);
    let back = at + egui::vec2(40.0, 0.0);
    h.frame(&mut state, vec![egui::Event::PointerMoved(back)]);
    h.frame(&mut state, vec![press(back, false)]);
    let _ = h.settle_folds(&mut state);
    assert!(
        !collapsed(&state, panes::Tab::Spectral),
        "a drag that came back off the floor should have left the analyzer open",
    );
    assert!(
        pane_width(&state, panes::Tab::Spectral) > 40.0,
        "and at the width it was dragged to, not {}",
        pane_width(&state, panes::Tab::Spectral),
    );
}

/// Downwards too: a stacked pane dragged to its floor folds the way egui_dock's
/// own arrow folds it, which needs nothing from `Folds` and moves no window.
#[test]
fn a_stacked_pane_dragged_down_to_its_floor_also_folds() {
    let mut h = DockHarness::new();
    let mut state = SharedState::new(TextureFormat::Bgra8Unorm);
    // The readouts open folded, so unfold them to have a vertical split with two
    // panes in it to drag between.
    unfold_the_readout_panes(&mut state);
    h.settle(&mut state);
    let style = theme::dock_style(&egui::Style::default(), 1.0);
    let readouts = pane_rect(&state, panes::Tab::Notes);
    let at = egui::pos2(readouts.center().x, readouts.top() - style.separator.width * 0.5);
    h.frame(&mut state, vec![egui::Event::PointerMoved(at)]);
    h.frame(&mut state, vec![egui::Event::PointerMoved(at), press(at, true)]);
    let target = at + egui::vec2(0.0, readouts.height() + 200.0);
    h.frame(&mut state, vec![egui::Event::PointerMoved(target)]);
    h.frame(&mut state, vec![egui::Event::PointerMoved(target)]);
    let floored = pane_rect(&state, panes::Tab::Notes).height();
    assert!(
        (floored - style.tab_bar.height).abs() < 1.0,
        "the floor downwards is a tab bar too, and left the readouts {floored} tall",
    );
    h.frame(&mut state, vec![press(target, false)]);
    assert!(
        collapsed(&state, panes::Tab::Notes),
        "letting go at the floor should have folded the readouts downwards",
    );
}

/// And its arrow gives back the height it had before the drag, not the tab bar's
/// worth the drag left it at — which would come back open and look folded.
#[test]
fn a_stacked_pane_folded_by_a_drag_comes_back_at_the_height_it_had() {
    let mut h = DockHarness::new();
    let mut state = SharedState::new(TextureFormat::Bgra8Unorm);
    unfold_the_readout_panes(&mut state);
    h.settle(&mut state);
    let separator = theme::dock_style(&egui::Style::default(), 1.0).separator.width;
    let readouts = pane_rect(&state, panes::Tab::Notes);
    let was = readouts.height();

    let at = egui::pos2(readouts.center().x, readouts.top() - separator * 0.5);
    h.frame(&mut state, vec![egui::Event::PointerMoved(at)]);
    h.frame(&mut state, vec![egui::Event::PointerMoved(at), press(at, true)]);
    let target = at + egui::vec2(0.0, readouts.height() + 200.0);
    h.frame(&mut state, vec![egui::Event::PointerMoved(target)]);
    h.frame(&mut state, vec![egui::Event::PointerMoved(target)]);
    h.frame(&mut state, vec![press(target, false)]);
    assert!(collapsed(&state, panes::Tab::Notes), "the drag should have folded the readouts");

    // The arrow on its tab bar, which is the only way back downwards.
    let _ = h.collapse_click(&mut state, panes::Tab::Notes);
    assert!(!collapsed(&state, panes::Tab::Notes), "the arrow should have brought them back");
    let now = pane_rect(&state, panes::Tab::Notes).height();
    assert!(
        (now - was).abs() < 2.0,
        "they should come back the {was} they were, not the {now} the drag left",
    );
}

/// Except in frameless mode, where a pane folded downwards has neither a
/// separator to drag back out of (egui_dock draws none) nor a tab bar to click an
/// arrow on: folding one there would be a door that only opens one way.
#[test]
fn a_stacked_pane_is_not_auto_folded_in_frameless_mode() {
    let mut h = DockHarness::new();
    let mut state = SharedState::new(TextureFormat::Bgra8Unorm);
    unfold_the_readout_panes(&mut state);
    ...
    assert!(
        !collapsed(&state, panes::Tab::Notes),
        "with no arrow and no separator to undo it, a drag must not fold a pane downwards",
    );
}
