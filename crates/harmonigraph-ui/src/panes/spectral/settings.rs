...
use crate::{Backdrop, KeylineStyle, PictureState};
...
pub(crate) fn spectrum_settings_pane(
    ...
    section(ui, "View", |ui| {
        ...
        if cfg.keyline_style != KeylineStyle::Off {
            ValueBar::new(&mut cfg.keyline_lift, 0.0..=1.0, "Spectrum outline lift").percent().show(ui).on_hover_text(
                "How bright the spectrum outline is kept. The outline takes the color of the spectrum under it, \
                     ...
                     100% is a white outline, 0% is exactly the color under it.",
            );
        }
        choice_row(ui, "Spectrum backdrop", &mut cfg.backdrop, &[
            ...
            (Backdrop::Gradient, "Gradient", "A smooth grey glow above the spectrum, brightest at the floor and fading upward. Softer than stripes, but a fill as bright as the glow at its own height blends into it."),
        ]);
        if cfg.backdrop != Backdrop::Off {
            ValueBar::new(&mut cfg.backdrop_strength, 0.0..=1.0, "Backdrop strength")
                .percent()
                .show(ui)
                .on_hover_text("How bright the backdrop is at the floor. Faint backdrops read as a flat wash; about 70% and up keep dark fills visible.");
        ...
        if cfg.backdrop == Backdrop::Stripes {
            ValueBar::new(&mut cfg.backdrop_period, crate::config::BACKDROP_PERIOD_RANGE, "Stripe spacing")
                .integer()
                .unit(1.0, " px")
                .show(ui)
                .on_hover_text("Pixels from one stripe to the next. 2 is every other pixel; wider spacing is calmer but can miss a partial one pixel wide.");
        }
    });
