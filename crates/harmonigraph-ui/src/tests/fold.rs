...
#[test]
fn a_section_fold_button_sits_as_far_from_its_first_tab_as_from_the_pane_edge() {
    for scale in [1.0, 1.5] {
        let mut state = fresh();
        let mut h = DockHarness::scaled(egui::vec2(1000.0, 800.0) * scale, scale, &mut state);
        h.settle(&mut state);
        let section = state.workspace.layout_runtime.rects[Section::Analyzer as usize];
        let out = h.frame(&mut state, vec![egui::Event::PointerGone]);
        let header = |r: &egui::Rect| {
            section.contains(r.center())
                && r.bottom() < section.top() + theme::tab_bar_height(scale)
        };
        let fold = out
            .shapes
            .iter()
            .find_map(|cs| match &cs.shape {
                egui::Shape::Rect(r) if header(&r.rect) && r.fill == theme::widget() => {
                    Some(r.rect)
                }
                _ => None,
            })
            .expect("header fold button");
        let label = out
            .shapes
            .iter()
            .find_map(|cs| match &cs.shape {
                egui::Shape::Text(t)
                    if t.galley.text() == "Analyzer" && section.contains(t.pos) =>
                {
                    Some(t.pos)
                }
                _ => None,
            })
            .expect("Analyzer tab label");
        let tab = out
            .shapes
            .iter()
            .find_map(|cs| match &cs.shape {
                egui::Shape::Rect(r) if header(&r.rect) && r.rect.contains(label) => Some(r.rect),
                _ => None,
            })
            .expect("selected Analyzer tab fill");
        let (edge, between) = (fold.left() - section.left(), tab.left() - fold.right());
        assert!(
            edge > 0.0 && (edge - between).abs() < 0.5,
            "{scale}: edge {edge} vs tab {between}"
        );
    }
}
