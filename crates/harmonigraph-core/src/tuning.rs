...
/// How far along its chain of fifths a keyboard generated by `fifth`
/// (microcents) puts primes 5 and 7: the position within fourteen fifths of C
/// nearest a just 5/4 and a just 7/4, ties to the shorter chain. Fourteen
/// rather than twelve so a Pythagorean keyboard's seventh is the 7/4 at C♭♭.
pub fn fifth_generated_steps(fifth: i32) -> [i32; 2] {
    let octave = i64::from(OCTAVE_MICROCENTS);
    [FIVE_JUST, SEVEN_JUST].map(|just| {
        let just = i64::from(microcents(just));
        (-14..=14)
            .min_by_key(|&k: &i32| {
                let d = (i64::from(k) * i64::from(fifth) - just).rem_euclid(octave);
                (d.min(octave - d), k.unsigned_abs())
            })
            .unwrap()
    })
}

/// The keyboard tuning a fifth alone implies, in microcents: the fifth, then
/// the chain positions [`fifth_generated_steps`] picks for 5 and 7.
pub fn fifth_generated(fifth: i32) -> [i32; 3] {
    let octave = i64::from(OCTAVE_MICROCENTS);
    let [five, seven] = fifth_generated_steps(fifth)
        .map(|k| (i64::from(k) * i64::from(fifth)).rem_euclid(octave) as i32);
    [fifth, five, seven]
}
...
mod tests {
    ...
    #[test]
    fn a_fifth_alone_spells_the_keyboards_third_and_seventh() {
        assert_eq!(fifth_generated(microcents(700.0)), [700_000_000, 400_000_000, 1_000_000_000]);
        for (fifth, steps, five, seven) in
            [(696.578, [4, 10], 386.31, 965.78), (701.955, [-8, -14], 384.36, 972.63)]
        {
            assert_eq!(fifth_generated_steps(microcents(fifth)), steps, "{fifth}");
            let [_, f, s] = fifth_generated(microcents(fifth)).map(|v| v as f32 / 1e6);
            assert!((f - five).abs() < 0.01 && (s - seven).abs() < 0.01, "{fifth}: {f} {s}");
        }
    }
