...
//!
//! Run with `cargo run -p harmonigraph-standalone`. A floating "MIDI input"
...
fn main() -> eframe::Result {
    ...
    let options = eframe::NativeOptions {
        ...
        viewport: egui::ViewportBuilder::default()
            ...
            .with_title("Harmonigraph — dev harness"),
        ..Default::default()
    };
    eframe::run_native(
        ...
        Box::new(|cc| {
            ...
            // the layout saved last run (eframe's storage, where the plugin
            // has the host's project). The release is a no-op in this harness,
            // whose state is built alongside the one context it will ever use
            // — but the day it grows a second window it stops being one, and
            // it is not this shell's call to make.
            let persist = cc.storage.and_then(|s| s.get_string(UI_STATE_STORAGE_KEY));
...
/// note event this harness sees, in the format `harmonigraph-offline` replays.
///
    ...
    fn params(&mut self, params: &StandaloneParams, now: f64) {
        for (i, key) in ParamKey::ALL.into_iter().enumerate() {
            ...
            if self.last[i] != value {
                self.last[i] = value;
                let _ = self.writer.param(harmonigraph_take::ParamRecord {
                    t: now,
                    id: key.id().to_string(),
                    value,
                });
...
impl App {
    ...
    fn connect(&mut self, name: &str) {
        ...
        let Ok(input) = midir::MidiInput::new("harmonigraph") else {
...
impl App {
    // eframe and harmonigraph-render resolve to the same wgpu version, so
    // eframe's reported target format can be passed straight through.
    fn new(target_format: harmonigraph_render::wgpu::TextureFormat) -> Self {
...
impl eframe::App for App {
    ...
    fn save(&mut self, storage: &mut dyn eframe::Storage) {
        storage.set_string(UI_STATE_STORAGE_KEY, harmonigraph_ui::shell::close(&self.state));
    }
}
...
impl MockSynth {
    ...
    fn render(&mut self, tracker: &harmonigraph_core::NoteTracker, now: f64, out: &mut Vec<f32>) {
        out.clear();
        ...
        let voices: Vec<(f64, f64)> = tracker
            ...
            .map(|v| {
                // harmonigraph_core::spectrum::midi_to_hz, in f64 (this synth's
