...
//     brings it back to;
//   - middle C is straight up, since the octave widths depend only on the
//     distance from it and so are symmetric about it;
...
fn oct_sector(s: u32, cents: f32) -> vec2<f32> {
    ...
    return vec2<f32>(oct_angle(p - 6.0), oct_angle(p + 6.0));
}
