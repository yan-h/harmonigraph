...
struct VsOut {
    ...
    @location(1) @interpolate(flat) thickness: vec3<u32>,
...
// spanning the node's outermost drawn edge plus Reach, with one light field,
...
fn glow_layer(light: vec2<f32>, uv: vec2<f32>) -> vec4<f32> {
    ...
    //
    // Use the ordinary ring edge for this transition, independently of the
    // larger fixed halo footprint. At the ring edge the colors are fully
    // directional; extending the mark rim must not wash them toward the mean.
    // A marked node uses the same transition, with no per-node size history.
    //
    ...
    let seam = max(node_rim(false), 0.1);
