...
/// do not touch.
///
/// This is the reading the bar had no test for while it did nothing. The colour
/// eased toward the strip's flat mean over the light's whole SPAN, so with any
/// real Reach dialled in the halo was that mean nearly everywhere — and the mean
/// is the one average the Spread bar cannot move, being taken at no
/// concentration at all by definition. Every setting drew one tint per node.
#[test]
fn the_glow_spread_says_how_separate_a_node_keeps_its_colours() {
    ...
    // The top of it is the mean, which has no direction left in it.
    assert!(
        spreads[2] < spreads[0] * 0.25,
        "the top of the bar must be one tint, and it kept {:.4} against {:.4}",
        spreads[2], spreads[0],
    );
}
