...
fn a_culled_node_behind_home_moves_no_marker() {
    ...
    scene.pluses = vec![one_marker(2, scene.nodes[2].world_pos, 0.2, scene.lattice_ground, 1.0)];
...
fn a_marker_nearer_the_eye_than_a_node_draws_after_it() {
    ...
    scene.pluses = [far, near]
        ...
        .zip([-3.0f32, 3.0])
        .enumerate()
        .map(|(node, (_, y))| harmonigraph_scene::PlusInstance {
            node,
            ..one_marker(node, glam::Vec3::new(0.0, y, 0.0), 0.2, scene.lattice_ground, 1.0)
        })
        .collect();
...
fn a_resting_lattice_ships_one_marker_draw() {
    ...
    scene.pluses = scene
        ...
        .map(|(node, n)| harmonigraph_scene::PlusInstance {
            node,
            ..one_marker(node, n.world_pos, 0.2, scene.lattice_ground, 1.0)
        })
        .collect();
...
/// A halo whose center is off-screen still contributes until its footprint
/// no longer reaches the pane. Read the actual picture, including clipping.
#[test]
fn offscreen_halos_draw_only_where_their_footprint_reaches() {
    const SIZE: [u32; 2] = [256, 256];
