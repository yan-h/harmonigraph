...
pub struct HarmonigraphParams {
    ...
    /// and it now drives every layer's arrival and release alike. Reusing
    /// the id means projects automating it keep their value instead of
    /// snapping back to the default. (The retired "octave-fade" id is simply
    /// ignored on load.)
    #[id = "pitch-class-fade"]
    pub fade: FloatParam,
    #[id = "darkest-pitch"]
