...
/// margin is: a node's own radius, the half step the center's rounding can be
/// off by, and a step for the name drawn beside it. Four steps of ring around
/// what shows is comfortably more than the margin asks for and far less than a
/// window that has stopped tracking the pane.
#[test]
...
/// Scrolling does not run out of lattice: pan a long way and the window is
/// still full, still the same size, and still centered on what the camera is
/// pointed at.
///
/// The whole feature in one test. Before the window was derived, panning this
/// far left the extents where they were and walked the lattice off the pane,
...
fn panning_a_long_way_keeps_the_window_full() {
    ...
    let aspect = 16.0 / 9.0;
    let at_home = view.scrolled(&camera, aspect).visible_count();

    // Two hundred steps out along both axes, in the drags a hand would make.
    for _ in 0..400 {
        camera.pan(Vec2::new(-120.0, -60.0));
        view.follow_camera(&mut camera);
    }
    let far = view.scrolled(&camera, aspect);
    assert_eq!(far.visible_count(), at_home, "the window changed size on the way out");
    assert!(
        far.center_fives.abs() > 20 && far.center_threes.abs() > 20,
        "the pan did not move the window far enough to be a test: {:?}",
        far.center(),
    );
    // The camera's own target is left inside one cell of the origin, which is
    // what keeps the number that would otherwise grow without bound small —
    // and small is what keeps a node's world position precise.
    assert!(
        camera.target.x.abs() <= view.spacing && camera.target.y.abs() <= view.spacing,
        "the target walked off with the window: {:?}",
        camera.target,
    );
    // The reach the names are chosen out of came along, so the notes on screen
    // are still ones the analyzer can name.
    let reach: std::collections::HashSet<_> = view.visible_positions().collect();
    for pos in far.visible_positions() {
        if ndc(&camera, &far, aspect, pos).is_some_and(|p| p.x.abs() <= 1.0 && p.y.abs() <= 1.0) {
            assert!(reach.contains(&pos), "{pos:?} is on screen and outside the naming reach");
        }
    }
}
