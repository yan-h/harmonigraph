...
, metal::texture2d<float, metal::access::sample> local_shadow_tex [[texture(3)]]
) {
    ...
    metal::float4 _e16 = local_shadow_tex.sample(scene_samp, in.uv);
    float t = metal::clamp(_e16.x, 0.0, 1.0);
    float _e27 = bu.bloom_strength;
    metal::float3 light = (scene.xyz + ink.xyz) + ((bloom.xyz * _e27) * t);
    metal::float4 _e33 = bu.background;
    metal::float3 ground = _e33.xyz * (1.0 - scene.w);
    metal::float3 capped = metal::max(metal::min(light + ground, metal::float3(t)) - ground, metal::float3(0.0));
    metal::float3 rgb_1 = (t >= 1.0) ? light : capped;
    bool has_light = metal::any(rgb_1 > metal::float3(0.0));
    metal::float3 _e55 = dither_to_unorm8_(rgb_1, in.pos.xy);
    metal::float3 dithered = has_light ? _e55 : rgb_1;
