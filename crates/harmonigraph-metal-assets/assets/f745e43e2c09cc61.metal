...
, metal::texture2d<float, metal::access::sample> glow_accumulated [[texture(2)]]
    ...
    uint clamped_lod_e13 = metal::min(uint(0), glow_sum.get_num_mip_levels() - 1);
    metal::float4 sum = glow_sum.read(metal::min(metal::uint2(pixel_1), metal::uint2(glow_sum.get_width(clamped_lod_e13), glow_sum.get_height(clamped_lod_e13)) - 1), clamped_lod_e13);
    uint clamped_lod_e16 = metal::min(uint(0), glow_screen.get_num_mip_levels() - 1);
    metal::float4 _e16 = glow_screen.read(metal::min(metal::uint2(pixel_1), metal::uint2(glow_screen.get_width(clamped_lod_e16), glow_screen.get_height(clamped_lod_e16)) - 1), clamped_lod_e16);
    metal::float2 bounded = _e16.xy;
    uint clamped_lod_e20 = metal::min(uint(0), glow_accumulated.get_num_mip_levels() - 1);
    metal::float4 accumulated = glow_accumulated.read(metal::min(metal::uint2(pixel_1), metal::uint2(glow_accumulated.get_width(clamped_lod_e20), glow_accumulated.get_height(clamped_lod_e20)) - 1), clamped_lod_e20);
    if (sum.w <= 1.0) {
        metal::float4 _e25 = nebula_light(accumulated, pos.xy, u);
        return fs_glow_resolveOutput { _e25 };
    }
    float _e29 = u.glow.accumulation;
    float accumulation = metal::clamp(_e29, 0.0, 1.0);
    if (accumulation >= 1.0) {
        metal::float4 _e36 = nebula_light(accumulated, pos.xy, u);
        return fs_glow_resolveOutput { _e36 };
    }
    float _e41 = u.glow.strength;
    float peak = metal::clamp(GLOW_BASE * _e41, 0.0, 1.0);
    metal::float3 _e47 = glow_linear(metal::float3(peak));
    float peak_luminance = _e47.x;
    ...
    if (total <= 0.0) {
        metal::float4 _e66 = nebula_light(metal::mix(metal::float4(0.0, 0.0, 0.0, peak * coverage), accumulated, accumulation), pos.xy, u);
        return fs_glow_resolveOutput { _e66 };
    }
    linear = rgb_2 * (light_1 / total);
    float _e71 = linear.x;
    float _e73 = linear.y;
    float _e76 = linear.z;
    float largest = metal::max(metal::max(_e71, _e73), _e76);
    if (largest > peak_luminance) {
        float chroma = metal::clamp((peak_luminance - light_1) / (largest - light_1), 0.0, 1.0);
        metal::float3 _e86 = linear;
        linear = metal::mix(metal::float3(light_1), _e86, chroma);
    }
    metal::float3 _e88 = linear;
    metal::float3 _e92 = glow_gamma(metal::max(_e88, metal::float3(0.0)));
    metal::float3 colour = metal::min(_e92, metal::float3(peak));
    float alpha = metal::max(peak * coverage, metal::max(metal::max(colour.x, colour.y), colour.z));
    metal::float4 _e106 = nebula_light(metal::mix(metal::float4(colour, metal::min(alpha, peak)), accumulated, accumulation), pos.xy, u);
    return fs_glow_resolveOutput { _e106 };
}
