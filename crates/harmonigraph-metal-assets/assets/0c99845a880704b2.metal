...
Painted plus_paint(
    ...
    metal::float2 _e65 = light_coord(in_1.clip_pos.xy, u);
    metal::float4 _e66 = glow_light(_e65, glow_tex, glow_sampler);
    metal::float3 _e69 = wash_over(ink_1, alpha_1, _e66.xyz, 1.0);
    return Painted {_e69, final_alpha, bloom_alpha, alpha_1};
}

struct fs_plus_transmittanceInput {
...
struct fs_plus_transmittanceOutput {
    metal::float4 member [[color(0)]];
};
fragment fs_plus_transmittanceOutput fs_plus_transmittance(
  fs_plus_transmittanceInput varyings [[stage_in]]
...
, constant _mslBufferSizes& _buffer_sizes [[buffer(3)]]
    ...
    return fs_plus_transmittanceOutput { metal::float4(_e1.ink_alpha, 0.0, 0.0, _e1.seen) };
}
