...
struct fs_blur_vInput {
    metal::float2 uv [[user(loc0), center_perspective]];
};
struct fs_blur_vOutput {
    metal::float4 member [[color(0)]];
};
fragment fs_blur_vOutput fs_blur_v(
  fs_blur_vInput varyings [[stage_in]]
...
, metal::sampler scene_samp [[sampler(0)]]
    ...
    metal::float4 _e5 = blur(in.uv, metal::float2(0.0, 1.0), scene_tex, scene_samp);
    return fs_blur_vOutput { _e5 };
}
