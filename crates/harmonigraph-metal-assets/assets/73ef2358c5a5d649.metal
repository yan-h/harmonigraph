...
, constant _mslBufferSizes& _buffer_sizes [[buffer(1)]]
    ...
    metal::float2 pos = (center + (_e55 * local.x)) + (_e61 * local.y);
    metal::float2 _e67 = locals.origin_points;
    metal::float2 in_viewport = pos - _e67;
    float _e77 = locals.viewport_points.x;
    float _e87 = locals.viewport_points.y;
    out.position = metal::float4(((2.0 * in_viewport.x) / _e77) - 1.0, 1.0 - ((2.0 * in_viewport.y) / _e87), 0.0, 1.0);
    out.local = local;
    out.half_extent = half_extent;
    out.shear = shear;
    float _e101 = locals.shadow.w;
    out.outline_reach = _e101;
    out.lead = lead;
