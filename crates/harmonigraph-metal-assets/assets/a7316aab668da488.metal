...
SectorFold sector_fold(
    ...
    return SectorFold {metal::float2(metal::abs((uv_1.y * c_2) - (uv_1.x * s_6)), (uv_1.x * c_2) + (uv_1.y * s_6)), metal::float2(metal::sin(half_), metal::cos(half_))};
}
...
float wedge_fraction(
    ...
    float c_3 = (uv_3.x * metal::cos(mid_2)) + (uv_3.y * metal::sin(mid_2));
    float s_7 = (-(uv_3.x) * metal::sin(mid_2)) + (uv_3.y * metal::cos(mid_2));
...
NodeLayer mark_extension(
    ...
    while(true) {
        ...
        }
        {
            ...
            bool _e58 = local_11;
            if (_e58) {
                NodeLayer _e59 = outer_glyph(s_8, ring_4, uv_5, strip, inner_3, outer_3, aa_5, u);
                float _e60 = sd_2;
