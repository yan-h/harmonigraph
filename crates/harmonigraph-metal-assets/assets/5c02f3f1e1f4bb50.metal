...
NodeGeom node_geom(
    ...
    if (ring_draws) {
        local_14 = metal::length(src.uv) >= (_e27.x - _e9);
    } else {
        local_14 = false;
    }
    bool _e39 = local_14;
    if (_e39) {
        local_15 = metal::length(src.uv) <= (_e27.y + _e9);
    } else {
        local_15 = false;
    }
    bool in_audio_ring = local_15;
    if (EARLY_OUT) {
        local_16 = !(analytic_2);
    } else {
        local_16 = false;
    ...
    bool _e54 = local_16;
    if (_e54) {
        local_17 = !(in_audio_ring);
    } else {
        local_17 = false;
    }
    bool _e59 = local_17;
    if (_e59) {
        local_18 = src.params.x <= 0.0;
    } else {
        local_18 = false;
    }
    bool _e67 = local_18;
    if (_e67) {
        local_19 = src.params.y <= 0.0;
    } else {
        local_19 = false;
    }
    bool _e75 = local_19;
    if (_e75) {
        local_20 = src.params.z <= 0.0;
    } else {
        local_20 = false;
    }
    bool _e83 = local_20;
    if (_e83) {
        local_21 = ((src.octaves.x | src.octaves.y) | src.octaves.z) == 0u;
    } else {
        local_21 = false;
    }
    bool _e97 = local_21;
    ...
    return NodeGeom {d_4, _e9, _e104, true};
}
...
NodeInk base_node_ink(
    ...
    if (band_out > band_in) {
        NodeLayer _e54 = glyph_band(d_1, band_in, band_out, 1.0, aa_6);
        band_2 = _e54;
    }
    ...
    while(true) {
        ...
        }
        {
            ...
            float _e83 = oct_slot_level(in_5.octaves, slot_3);
            if (_e83 <= 0.0) {
                local_25 = presence_1 <= 0.0;
            } else {
                local_25 = false;
            }
            bool _e91 = local_25;
            ...
            NodeLayer _e94 = outer_glyph(slot_3, oct_1, in_5.uv, _e93, band_in, band_out, aa_6, u);
            float _e95 = layer_coverage(_e94);
            float _e96 = glyph_mask;
            glyph_mask = metal::max(_e96, _e94.coverage);
            metal::float4 _e99 = oct_slot_ink(in_5, slot_3, u);
    ...
    glyph_mask = _e123 * glyph_taper;
    {
        ...
        NodeLayer _e130 = glyph_band(d_1, _e125.x, _e125.y, 1.0, aa_6);
        RingInk _e131 = spectral_ring(in_5, oct_1, in_5.uv, _e130, aa_6, analytic_3, u);
        float _e132 = node_sd;
        ...
        float _e176 = mask_level(in_5.ring);
        float audio_mask = _e131.layer.coverage * _e176;
        float _e178 = glyph_mask;
        glyph_mask = audio_mask + (_e178 * (1.0 - audio_mask));
    }
    if (mark_out > mark_in) {
        NodeLayer _e190 = glyph_band(d_1, mark_in, mark_out, 1.0, aa_6);
        mark_strip = _e190;
    }
    float _e195 = mark_strip.sd;
    float _e199 = mark_strip.coverage;
    NodeLayer _e201 = mark_extension(in_5.marks.x, oct_1, in_5.uv, NodeLayer {_e195, in_5.params.y, _e199}, mark_in, mark_out, aa_6, analytic_3, u);
    float _e206 = mark_strip.sd;
    float _e210 = mark_strip.coverage;
    NodeLayer _e212 = mark_extension(in_5.marks.y, oct_1, in_5.uv, NodeLayer {_e206, in_5.params.z, _e210}, mark_in, mark_out, aa_6, analytic_3, u);
    ...
    metal::float3 mark_rgb = (_e213 > _e214) ? in_5.melody_color.xyz : in_5.bass_color.xyz;
...
float slice_progress(
    ...
    return static_cast<float>((in_6.motion[metal::min(unsigned(naga_div(i_4, 3u)), 3u)] >> (naga_mod(i_4, 3u) * 10u)) & 1023u) / 1023.0;
}
...
AnimatedInk animated_slice_ink(
    ...
    while(true) {
        ...
        {
            uint _e50 = i_5;
            int slot_4 = as_type<int>(as_type<uint>(oct_2.base) + as_type<uint>(static_cast<int>(_e50)));
            uint _e53 = i_5;
            float _e54 = slice_progress(in_7, _e53);
            if (_e54 <= 0.0) {
                continue;
            }
            float _e57 = motion_ease(_e54, u);
            float _e62 = u.node.pose.x;
            float _e69 = u.node.pose.y;
            ...
            metal::float2 uv_6 = anchor + ((in_7.uv - start) / metal::float2(scale));
            float d_5 = metal::length(uv_6);
            float soft = aa_7 / scale;
            if (band_out_1 > band_in_1) {
                NodeLayer _e118 = glyph_band(d_5, band_in_1, band_out_1, 1.0, soft);
                NodeLayer _e119 = outer_glyph(slot_4, oct_2, uv_6, _e118, band_in_1, band_out_1, soft, u);
                metal::float4 _e120 = oct_slot_ink(in_7, slot_4, u);
                float taper = 1.0 - metal::smoothstep(1.0, GLYPH_FADE_LIMIT, d_5);
                float coverage_1 = ((_e119.coverage * taper) * _e120.w) * opacity_2;
                float _e132 = result.alpha;
                if (coverage_1 > _e132) {
                    result.rgb = _e120.xyz * coverage_1;
                    result.alpha = coverage_1;
                    float _e140 = oct_slot_level(in_7.octaves, slot_4);
                ...
                float _e164 = layer_distance(_e157, NodeLayer {_e119.sd * scale, _e120.w * opacity_2, _e119.coverage});
                result.sd = _e164;
            }
            if (slot_4 >= 0) {
                local_26 = slot_4 < 11;
            ...
            if (_e177) {
                ...
                float level_4 = metal::max(melody, bass) * opacity_2;
                metal::float3 color = (melody > bass) ? in_7.melody_color.xyz : in_7.bass_color.xyz;
                NodeLayer _e207 = glyph_band(d_5, mark_in_1, mark_out_1, level_4, soft);
                NodeLayer _e208 = outer_glyph(slot_4, oct_2, uv_6, _e207, mark_in_1, mark_out_1, soft, u);
                float taper_1 = 1.0 - metal::smoothstep(1.5600001, QUAD_MARGIN, d_5);
                float _e214 = layer_coverage(_e208);
                float coverage_2 = _e214 * taper_1;
                float _e217 = marks.alpha;
                if (coverage_2 > _e217) {
                    marks.rgb = color * coverage_2;
                    marks.alpha = coverage_2;
                    marks.lit = 1.0;
                }
                float _e226 = marks.mask;
                float _e229 = mask_level(level_4);
                marks.mask = metal::max(_e226, (_e208.coverage * taper_1) * _e229);
                float _e234 = marks.sd;
                float _e239 = layer_distance(_e234, NodeLayer {_e208.sd * scale, level_4, _e208.coverage});
                marks.sd = _e239;
            }
        }
    }
...
NodeInk node_ink(
    ...
    if (_e8 == 0.0) {
        NodeInk _e11 = base_node_ink(src_1, d_2, aa_8, oct_3, analytic_4, u);
        return _e11;
    }
    ...
    bool _e41 = local_29;
    if (_e41) {
        NodeInk _e42 = base_node_ink(src_1, d_2, aa_8, oct_3, analytic_4, u);
        return _e42;
    }
    ...
    float _e59 = ink.lit;
    float _e61 = ink.alpha;
    float lit = (_e54.lit * _e54.cov) + ((_e59 * _e61) * (1.0 - _e54.cov));
    metal::float3 _e73 = ink.rgb;
    ink.rgb = (_e54.color * _e54.cov) + (_e73 * (1.0 - _e54.cov));
    float _e82 = ink.alpha;
    ink.alpha = _e54.cov + (_e82 * (1.0 - _e54.cov));
    float _e90 = ink.alpha;
    ink.lit = lit / metal::max(_e90, 0.0001);
    float _e97 = mask_level(src_1.ring);
    float mask = _e54.layer.coverage * _e97;
    float _e101 = ink.mask;
    ink.mask = mask + (_e101 * (1.0 - mask));
    float _e108 = ink.sd;
    float _e110 = layer_distance(_e108, _e54.layer);
    ink.sd = _e110;
    float _e114 = ink.lit;
    float _e116 = ink.alpha;
    float mark_lit = _e43.marks.alpha + ((_e114 * _e116) * (1.0 - _e43.marks.alpha));
    metal::float3 _e128 = ink.rgb;
    ink.rgb = _e43.marks.rgb + (_e128 * (1.0 - _e43.marks.alpha));
    float _e139 = ink.alpha;
    ink.alpha = _e43.marks.alpha + (_e139 * (1.0 - _e43.marks.alpha));
    float _e148 = ink.alpha;
    ink.lit = mark_lit / metal::max(_e148, 0.0001);
    float _e156 = ink.mask;
    ink.mask = _e43.marks.mask + (_e156 * (1.0 - _e43.marks.mask));
    float _e165 = ink.sd;
    ink.sd = metal::min(_e165, _e43.marks.sd);
    NodeInk _e169 = ink;
    return _e169;
}
