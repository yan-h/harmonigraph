...
metal::uint4 star_bake(
    ...
    if (_e23.z >= s.occupancy) {
        return metal::uint4(0u);
    }
    float through = metal::fract(age);
    metal::float2 spread_1 = s.spread * (((2.0 * _e23.w) - 1.0) * (through - 0.5));
    metal::float2 centre = (metal::float2(0.5) + (STAR_JITTER * (_e23.xy - metal::float2(0.5)))) + spread_1;
    float _e69 = cloud.size.y;
    metal::float2 _e75 = cloud.size;
    metal::float2 at = ((((((static_cast<metal::float2>(cell_1) + metal::float2(0.5)) + (STAR_JITTER * (_e23.xy - metal::float2(0.5)))) + spread_1) + s.offset) * s.cell) * (_e69 / STAR_PANE)) + (_e75 * 0.5);
    float _e79 = star_level_at(at, close_light, cloud_sampler, cloud);
    if (_e79 <= 0.0) {
        return metal::uint4(0u);
    }
    metal::float4 _e86 = star_hash(hashed, key + 1u);
    float randomness_1 = cloud.star_randomness;
    metal::float3 _e101 = star_paint(_e79, metal::pow(_e86.x, 1.0 + (6.0 * randomness_1)) * (2.0 + (6.0 * randomness_1)), lut, cloud);
    float size = metal::exp(((0.3 + (0.9 * randomness_1)) * (_e86.y - 0.5)) * 2.0);
    ...
    metal::uint3 tens = naga_f2u32(metal::rint(metal::clamp(_e101, metal::float3(0.0), metal::float3(1.0)) * 1023.0));
