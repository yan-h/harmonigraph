...
struct fs_label_transmittanceInput {
    metal::float2 at [[user(loc0), center_perspective]];
    float level [[user(loc1), flat]];
    uint who [[user(loc2), flat]];
};
struct fs_label_transmittanceOutput {
    metal::float4 member [[color(0)]];
};
fragment fs_label_transmittanceOutput fs_label_transmittance(
  fs_label_transmittanceInput varyings [[stage_in]]
...
, constant _mslBufferSizes& _buffer_sizes [[buffer(3)]]
    ...
    return fs_label_transmittanceOutput { metal::float4(0.0, 0.0, 0.0, 1.0 - _e8) };
}
