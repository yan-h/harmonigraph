...
, metal::texture2d<float, metal::access::sample> ink_strip [[texture(0)]]
    ...
    while(true) {
        ...
        {
            uint _e17 = i;
            uint offset = static_cast<uint>(naga_abs(as_type<int>(as_type<uint>(static_cast<int>(_e17)) - as_type<uint>(metal::min(col, 63)))));
            float _e32 = u.ink_kernel.inner[metal::min(unsigned(naga_div(offset, 4u)), 15u)][metal::min(unsigned(naga_mod(offset, 4u)), 3u)];
            float lobe = (col >= 64) ? 1.0 : _e32;
            uint _e38 = i;
            uint clamped_lod_e42 = metal::min(uint(0), ink_strip.get_num_mip_levels() - 1);
            metal::float4 ink = ink_strip.read(metal::min(metal::uint2(metal::int2(static_cast<int>(_e38), row)), metal::uint2(ink_strip.get_width(clamped_lod_e42), ink_strip.get_height(clamped_lod_e42)) - 1), clamped_lod_e42);
            metal::float3 _e43 = rgb;
            rgb = _e43 + (ink.xyz * lobe);
            float _e47 = wsum;
            wsum = _e47 + (ink.w * lobe);
            float _e51 = lobes;
            lobes = _e51 + lobe;
        }
    }
    metal::float3 _e56 = rgb;
    float _e57 = wsum;
    float _e62 = wsum;
    float _e63 = lobes;
    return fs_ink_blurOutput { metal::float4(_e56 / metal::float3(metal::max(_e57, 0.00001)), _e62 / metal::max(_e63, 0.00001)) };
}
