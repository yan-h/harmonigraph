...
SectorFold sector_fold(
    ...
    return SectorFold {metal::float2(metal::abs((uv_1.y * c_1) - (uv_1.x * s_5)), (uv_1.x * c_1) + (uv_1.y * s_5)), metal::float2(metal::sin(half_), metal::cos(half_))};
}
...
float wedge_fraction(
    ...
    float s_6 = (-(uv_3.x) * metal::sin(mid_2)) + (uv_3.y * metal::cos(mid_2));
...
NodeLayer mark_extension(
    ...
    while(true) {
        ...
        }
        {
            ...
            bool _e58 = local_11;
            if (_e58) {
                NodeLayer _e59 = outer_glyph(s_7, ring_4, uv_5, strip, inner_3, outer_3, aa_5, u);
                float _e60 = sd_2;
...
NodeInk slice_pop_ink(
    ...
    while(true) {
        ...
        }
        {
            ...
            metal::float2 uv_6 = anchor + ((in_6.uv - anchor) / metal::float2(_e56));
            float d_5 = metal::length(uv_6);
            float soft = aa_7 / _e56;
            if (band_out_1 > band_in_1) {
                NodeLayer _e73 = glyph_band(d_5, band_in_1, band_out_1, 1.0, soft);
                NodeLayer _e74 = outer_glyph(slot_4, oct_3, uv_6, _e73, band_in_1, band_out_1, soft, u);
                metal::float4 _e75 = oct_slot_ink(in_6, slot_4, u);
                float taper = 1.0 - metal::smoothstep(1.0, GLYPH_FADE_LIMIT, d_5);
                float coverage_1 = (_e74.coverage * taper) * _e75.w;
                float _e86 = result_1.alpha;
                if (coverage_1 > _e86) {
                    result_1.rgb = _e75.xyz * coverage_1;
                    result_1.alpha = coverage_1;
            ...
            if (_e129) {
                ...
                float level_4 = metal::max(melody, bass);
                metal::float3 color = (melody > bass) ? in_6.melody_color.xyz : in_6.bass_color.xyz;
                NodeLayer _e158 = glyph_band(d_5, mark_in_1, mark_out_1, level_4, soft);
                NodeLayer _e159 = outer_glyph(slot_4, oct_3, uv_6, _e158, mark_in_1, mark_out_1, soft, u);
                float taper_1 = 1.0 - metal::smoothstep(1.5600001, QUAD_MARGIN, d_5);
                float _e165 = layer_coverage(_e159);
                float coverage_2 = _e165 * taper_1;
                float _e168 = result_1.alpha;
                if (coverage_2 > _e168) {
                    result_1.rgb = color * coverage_2;
                    result_1.alpha = coverage_2;
                    result_1.lit = 1.0;
                }
                float _e177 = result_1.mask;
                float _e180 = mask_level(level_4);
                result_1.mask = metal::max(_e177, (_e159.coverage * taper_1) * _e180);
                float _e185 = result_1.sd;
                float _e190 = layer_distance(_e185, NodeLayer {_e159.sd * _e56, level_4, _e159.coverage});
                result_1.sd = _e190;
            }
        }
    }
...
NodeInk node_ink(
    ...
    if (mode >= 6.0) {
        local_29 = metal::abs(src_2.params.w) != 1.0;
    } else {
        local_29 = false;
    }
    bool _e21 = local_29;
    if (_e21) {
        ...
        NodeInk _e23 = base_node_ink(_e5, d_2, aa_8, oct_4, analytic_4, u);
        ink = _e23;
    }
    if (mode == 0.0) {
        NodeInk _e26 = ink;
    ...
    if (mode == 1.0) {
        ...
        ink.sd = _e33 * metal::max(_e34, 0.05);
    }
    if (mode == 2.0) {
        local_30 = p_2 < 1.0;
    } else {
    ...
    if (_e45) {
        ...
        float edge_2 = (((angle <= p_2) ? -1.0 : 1.0) * d_2) * metal::sin(metal::min(nearest_1 * TAU, 1.5707964));
        float coverage_3 = (p_2 > 0.0) ? (1.0 - metal::smoothstep(-(aa_8), aa_8, edge_2)) : 0.0;
        metal::float3 _e85 = ink.rgb;
        ink.rgb = _e85 * coverage_3;
        float _e88 = ink.alpha;
        ink.alpha = _e88 * coverage_3;
        float _e91 = ink.mask;
        ink.mask = _e91 * coverage_3;
        float _e95 = ink.sd;
        ink.sd = metal::max(_e95, edge_2);
    }
    if (mode == 4.0) {
        float _e100 = ink.mask;
        ink.mask = _e100 * (p_2 * p_2);
    }
    ...
    if (_e219 > 0.0) {
        ...
        float _e229 = accent;
        metal::float3 _e232 = ink.rgb;
        float _e233 = accent;
        ink.rgb = (rgb * _e229) + (_e232 * (1.0 - _e233));
        float _e239 = accent;
        float _e241 = ink.alpha;
        float _e242 = accent;
        ink.alpha = _e239 + (_e241 * (1.0 - _e242));
        float _e249 = ink.mask;
        float _e250 = accent;
        ink.mask = metal::max(_e249, _e250);
        float _e252 = accent;
        if (_e252 >= 0.5) {
            float _e257 = ink.sd;
            float _e258 = accent_sd;
            ink.sd = metal::min(_e257, _e258);
        }
    }
    ...
    float _e277 = ink.lit;
    float _e279 = ink.alpha;
    float lit = (_e272.lit * _e272.cov) + ((_e277 * _e279) * (1.0 - _e272.cov));
    metal::float3 _e291 = ink.rgb;
    ink.rgb = (_e272.color * _e272.cov) + (_e291 * (1.0 - _e272.cov));
    float _e300 = ink.alpha;
    ink.alpha = _e272.cov + (_e300 * (1.0 - _e272.cov));
    float _e308 = ink.alpha;
    ink.lit = lit / metal::max(_e308, 0.0001);
    float _e315 = mask_level(src_2.ring);
    float mask = _e272.layer.coverage * _e315;
    float _e319 = ink.mask;
    ink.mask = mask + (_e319 * (1.0 - mask));
    float _e326 = ink.sd;
