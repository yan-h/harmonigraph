...
float bucket_level(
    ...
    float level_5 = (_e18 + (_e21 * v)) + (_e26 * midi_1);
...
metal::int2 wrap_cell_for_tile(
    ...
    metal::int2 positive_mod = naga_mod(as_type<metal::int2>(as_type<metal::uint2>(naga_mod(cell, metal::int2(period))) + as_type<metal::uint2>(metal::int2(period))), metal::int2(period));
    int shift = naga_f2i32(metal::rint((static_cast<float>(period) * CLOUD_TILE_PITCH_SHIFT) / static_cast<float>(tile_cells)));
    if (pitch_vertical == 1u) {
        int repeat = naga_div(as_type<int>(as_type<uint>(cell.x) - as_type<uint>(positive_mod.x)), period);
        return metal::int2(positive_mod.x, naga_mod(as_type<int>(as_type<uint>(naga_mod(as_type<int>(as_type<uint>(cell.y) - as_type<uint>(as_type<int>(as_type<uint>(repeat) * as_type<uint>(shift)))), period)) + as_type<uint>(period)), period));
    }
    int repeat_1 = naga_div(as_type<int>(as_type<uint>(cell.y) - as_type<uint>(positive_mod.y)), period);
    return metal::int2(naga_mod(as_type<int>(as_type<uint>(naga_mod(as_type<int>(as_type<uint>(cell.x) - as_type<uint>(as_type<int>(as_type<uint>(repeat_1) * as_type<uint>(shift)))), period)) + as_type<uint>(period)), period), positive_mod.y);
}
...
metal::int2 wrap_cell(
    ...
    uint _e4 = cloud.tile_cells;
    uint _e7 = cloud.pitch_vertical;
    metal::int2 _e8 = wrap_cell_for_tile(cell_1, period_1, _e4, _e7);
    return _e8;
}
...
metal::float2 cloud_tile_uv(
    ...
    float period_9 = static_cast<float>(_e3);
    uint _e7 = cloud.pitch_vertical;
    if (_e7 == 1u) {
        float time = r.x / period_9;
        return metal::float2(time, (r.y - (time * CLOUD_TILE_PITCH_SHIFT)) / period_9);
    }
    float time_1 = r.y / period_9;
    return metal::float2(time_1, (r.x - (time_1 * CLOUD_TILE_PITCH_SHIFT)) / period_9);
}
...
Pile dome_octave(
    ...
    float gain = {};
    Pile out = {};
    metal::float2 base = metal::floor(r_1);
        ...
        }
        {
            ...
            while(true) {
                ...
                }
                {
                    ...
                    metal::int2 _e26 = wrap_cell(cell_5, period_2, cloud);
                    ...
                    metal::float2 d = (r_1 - centre) / metal::float2(radius);
...
Pile cloud_domes(
    ...
    Pile out_1 = {};
    Pile _e2 = dome_octave(r_2, period_3, cloud);
    Pile _e14 = dome_octave((r_2 * DOME_LACUNARITY) + metal::float2(17.3, 5.9), naga_f2i32(metal::rint(DOME_LACUNARITY * static_cast<float>(period_3))), cloud);
...
float scale_tone(
    ...
    metal::float2 r_8 = q_1 * scale_units;
    uint _e35 = cloud.tile_cells;
    if (_e35 > 0u) {
        metal::float2 _e40 = cloud_tile_uv(r_8, cloud);
        metal::float4 tile = cloud_tile_a.sample(tile_sampler, _e40, metal::level(0.0));
        pile.face = tile.xy;
        pile.to_centre = tile.zw;
    } else {
        Pile _e48 = cloud_domes(r_8, 0, cloud);
        pile = _e48;
    }
    metal::float2 face_1 = pile.face;
    float _e53 = cloud.scale_refract;
    float bend = metal::max(_e53, 0.0) * scale_points;
    float _e59 = cloud.scale_refract;
    float gather = metal::max(-(_e59), 0.0) * scale_points;
    metal::float2 _e67 = pile.to_centre;
    metal::float2 lookup = (-(face_1) * bend) + (_e67 * gather);
    float _e71 = cloud_light(pt_1 + lookup, close_light, wide_light, cloud_sampler, cloud);
    metal::float2 reach = metal::float2(scale_points * 0.75, 0.0);
    float _e78 = cloud_light(pt_1 + reach.xy, close_light, wide_light, cloud_sampler, cloud);
    float _e81 = cloud_light(pt_1 - reach.xy, close_light, wide_light, cloud_sampler, cloud);
    float _e85 = cloud_light(pt_1 + reach.yx, close_light, wide_light, cloud_sampler, cloud);
    float _e88 = cloud_light(pt_1 - reach.yx, close_light, wide_light, cloud_sampler, cloud);
    metal::float2 grad = metal::float2(_e78 - _e81, _e85 - _e88);
    ...
    float lit = (_e71 * diffuse) * CLOUD_SHADE;
...
float wash_noise(
    ...
    metal::int2 _e13 = wrap_cell(i_4, period_4, cloud);
    metal::float3 _e14 = wash_hash(_e13, salt_1);
    float n00_ = _e14.x;
    metal::int2 _e20 = wrap_cell(as_type<metal::int2>(as_type<metal::uint2>(i_4) + as_type<metal::uint2>(metal::int2(1, 0))), period_4, cloud);
    metal::float3 _e21 = wash_hash(_e20, salt_1);
    float n10_ = _e21.x;
    metal::int2 _e27 = wrap_cell(as_type<metal::int2>(as_type<metal::uint2>(i_4) + as_type<metal::uint2>(metal::int2(0, 1))), period_4, cloud);
    metal::float3 _e28 = wash_hash(_e27, salt_1);
    float n01_ = _e28.x;
    metal::int2 _e34 = wrap_cell(as_type<metal::int2>(as_type<metal::uint2>(i_4) + as_type<metal::uint2>(metal::int2(1, 1))), period_4, cloud);
...
float wash_fbm(
    ...
    float _e8 = wash_noise(p_1, salt_2, period_5, cloud);
    float _e18 = wash_noise((p_1 * lacunarity) + metal::float2(13.1, -7.3), salt_2 + 31u, as_type<int>(as_type<uint>(period_5) * as_type<uint>(2)), cloud);
...
Glob wash_glob(
    ...
    Glob out_2 = {};
    metal::int2 _e5 = wrap_cell(cell_4, period_6, cloud);
    ...
    out_2.edge = metal::length(r_3 - centre_1) / radius_1;
    Glob _e39 = out_2;
    return _e39;
}
...
Wash wash_scan(
    ...
    front_a = r_4;
    front_b = r_4;
    metal::int2 base_1 = naga_f2i32(metal::floor(r_4));
        ...
        }
        {
            ...
            while(true) {
                ...
                }
                {
                    int _e40 = i_1;
                    int _e41 = j_1;
                    Glob _e44 = wash_glob(as_type<metal::int2>(as_type<metal::uint2>(base_1) + as_type<metal::uint2>(metal::int2(_e40, _e41))), salt_4, r_4, occupancy_1, period_7, cloud);
...
Wet wash_wet(
    ...
    float _e98 = pigment;
    return Wet {_e96 - r_5, metal::max(_e98, 0.0)};
}
...
WashField wash_field(
    ...
    if (_e6 > 0.0) {
        ...
        float _e25 = wash_fbm(r_6 * WASH_WARP_SCALE, 71u, warp_period, cloud);
        float _e35 = wash_fbm((r_6 * WASH_WARP_SCALE) + metal::float2(37.0, -19.0), 73u, warp_period, cloud);
        warped = _e19 + ((amp * 2.0) * metal::float2(_e25 - 0.5, _e35 - 0.5));
    }
    metal::float2 _e43 = warped;
    Wash _e46 = wash_scan(_e43, 1u, 1.0, period_8, cloud);
    ...
    if (want_fine) {
        ...
        Wash _e70 = wash_scan(fine_r, 2u, WASH_FINE_OCCUPANCY, naga_f2i32(metal::rint(WASH_LACUNARITY * static_cast<float>(period_8))), cloud);
...
WashField wash_tile_field(
    ...
    WashField out_5 = {};
    metal::float2 _e1 = cloud_tile_uv(r_7, cloud);
    metal::float4 a_1 = cloud_tile_a.sample(tile_sampler, _e1, metal::level(0.0));
    out_5.coarse = Wet {a_1.xy, a_1.z};
    ...
    if (_e20 > 0.0) {
        ...
        out_5.fine = Wet {b_5.xy, b_5.z};
        out_5.cover = b_5.w;
    }
    WashField _e33 = out_5;
    return _e33;
}
...
float wash_cloud_tone(
    ...
    metal::float2 r_9 = q_2 * cells;
    uint _e35 = cloud.tile_cells;
    if (_e35 > 0u) {
        WashField _e38 = wash_tile_field(r_9, cloud, cloud_tile_a, cloud_tile_b, tile_sampler);
        field = _e38;
    } else {
        float _e42 = cloud.wash_layers;
        WashField _e45 = wash_field(r_9, 0, _e42 > 0.0, cloud);
        field = _e45;
    }
