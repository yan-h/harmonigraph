...
float sheet_alpha(
    ...
    metal::float2 uv_1 = texel_1 / in_2.sheet_size;
    if (in_2.sheet == SHEET_MARK) {
        metal::float4 _e10 = mark_atlas.sample(atlas_sampler, uv_1, metal::level(0.0));
        return _e10.w;
    }
    metal::float4 _e15 = atlas.sample(atlas_sampler, uv_1, metal::level(0.0));
    return _e15.w;
}
...
metal::float4 glyph_light(
    ...
    metal::float2 _e3 = locals.screen_points;
    metal::float4 _e5 = glow_light(points_2 / _e3, glow_tex, glow_sampler);
    return _e5;
}
...
, metal::sampler glow_sampler [[sampler(1)]]
...
, constant _mslBufferSizes& _buffer_sizes [[buffer(2)]]
    ...
    metal::float4 _e16 = glyph_light(in.points, glow_tex, glow_sampler, locals);
    metal::float3 _e25 = wash_over(ink_1.xyz, ink_1.w, _e16.xyz, 1.0);
    const auto _tmp = SplitOut {metal::float4(0.0, 0.0, 0.0, alpha_1), metal::float4(_e25 * _e10, alpha_1)};
