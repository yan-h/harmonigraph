...
SectorFold sector_fold(
    ...
    return SectorFold {metal::float2(metal::abs((uv_1.y * c_2) - (uv_1.x * s_8)), (uv_1.x * c_2) + (uv_1.y * s_8)), metal::float2(metal::sin(half_), metal::cos(half_))};
}
...
float annular_sector_distance(
    ...
    if (radius >= inner_2) {
        local_4 = radius <= outer_2;
    } else {
        local_4 = false;
    }
    bool _e77 = local_4;
    if (_e77) {
        local_5 = (metal::dot(normal, f_2.q) + gap) <= 0.0;
    } else {
        local_5 = false;
    }
    bool inside = local_5;
...
SliceZones slice_zones(
    ...
    if (reach_1 > inner_4) {
        ...
        NodeLayer _e18 = outer_glyph(s_6, ring_4, uv_3, _e17, inner_4, reach_1, aa_4, u);
        lit = _e18;
    }
    NodeLayer _e19 = lit;
    float _e20 = layer_coverage(_e19);
    float lit_cov = _e20 * ink.w;
    float _e23 = layer_coverage(full);
    NodeLayer _e24 = lit;
    ...
    metal::float3 rgb_1 = ((ink.xyz * lit_cov) + (_e37.xyz * ghost_cov)) / metal::float3(metal::max(cov_2, 0.0001));
    NodeLayer _e46 = lit;
...
float wedge_fraction(
    ...
    float s_9 = (-(uv_4.x) * metal::sin(mid_2)) + (uv_4.y * metal::cos(mid_2));
...
RingInk spectral_ring(
    ...
    while(true) {
        ...
        }
        {
            ...
            NodeLayer _e70 = outer_glyph(slot_2, oct, uv_5, band_1, _e6.x, _e6.y, aa_5, u);
...
NodeLayer mark_extension(
    ...
    while(true) {
        ...
        }
        {
            ...
            bool _e58 = local_12;
            if (_e58) {
                NodeLayer _e59 = outer_glyph(s_10, ring_5, uv_6, strip, inner_5, outer_4, aa_6, u);
                float _e60 = sd_2;
...
NodeGeom node_geom(
    ...
    if (ring_draws) {
        local_15 = metal::length(src.uv) >= (_e27.x - _e9);
    } else {
        local_15 = false;
    }
    bool _e39 = local_15;
    if (_e39) {
        local_16 = metal::length(src.uv) <= (_e27.y + _e9);
    } else {
        local_16 = false;
    }
    ...
    bool _e59 = local_18;
    if (_e59) {
        local_19 = src.params.x <= 0.0;
    } else {
        local_19 = false;
    }
    bool _e67 = local_19;
    if (_e67) {
        local_20 = src.params.y <= 0.0;
    } else {
        local_20 = false;
    }
    bool _e75 = local_20;
    if (_e75) {
        local_21 = src.params.z <= 0.0;
    } else {
        local_21 = false;
    }
    bool _e83 = local_21;
    if (_e83) {
        local_22 = ((src.octaves.x | src.octaves.y) | src.octaves.z) == 0u;
    } else {
        local_22 = false;
    }
    bool _e97 = local_22;
...
NodeInk base_node_ink(
    ...
    while(true) {
        ...
        }
        {
            ...
            float _e83 = oct_slot_level(in_8.octaves, slot_3);
            if (_e83 <= 0.0) {
            ...
            NodeLayer _e94 = outer_glyph(slot_3, oct_1, in_8.uv, _e93, band_in, band_out, aa_7, u);
            float _e95 = layer_coverage(_e94);
            float _e96 = glyph_mask;
            glyph_mask = metal::max(_e96, _e94.coverage);
            metal::float4 _e99 = oct_slot_ink(in_8, slot_3, u);
            float opacity_1 = _e99.w;
            slot_rgb = _e99.xyz;
            cov_1 = _e95 * opacity_1;
            lit_shape = _e95;
            float _e106 = slice_reach(in_8, slot_3, band_in, band_out);
            if (_e106 >= band_out) {
                float _e108 = node_sd;
                float _e112 = layer_distance(_e108, NodeLayer {_e94.sd, opacity_1, _e94.coverage}, in_8);
                node_sd = _e112;
            } else {
                SliceZones _e114 = slice_zones(in_8, slot_3, oct_1, in_8.uv, d_3, _e94, _e99, band_in, _e106, aa_7, u);
...
AnimatedInk animated_slice_ink(
    ...
    while(true) {
        ...
        }
        {
            ...
            metal::float2 uv_7 = anchor + ((in_10.uv - start) / metal::float2(scale));
            float d_7 = metal::length(uv_7);
            float soft = aa_8 / scale;
            if (band_out_1 > band_in_1) {
                NodeLayer _e93 = glyph_band(d_7, band_in_1, band_out_1, 1.0, soft);
                NodeLayer _e94 = outer_glyph(slot_4, oct_2, uv_7, _e93, band_in_1, band_out_1, soft, u);
                metal::float4 _e95 = oct_slot_ink(in_10, slot_4, u);
                float taper = 1.0 - metal::smoothstep(1.0, GLYPH_FADE_LIMIT, d_7);
                float _e102 = oct_slot_level(in_10.octaves, slot_4);
                coverage_1 = ((_e94.coverage * taper) * _e95.w) * _e54;
                rgb = _e95.xyz;
                lit_1 = _e102 / metal::max(_e95.w, 0.0001);
                float _e116 = slice_reach(in_10, slot_4, band_in_1, band_out_1);
                if (_e116 >= band_out_1) {
                    float _e120 = result.sd;
                    float _e127 = layer_distance(_e120, NodeLayer {_e94.sd * scale, _e95.w * _e54, _e94.coverage}, in_10);
                    result.sd = _e127;
                } else {
                    SliceZones _e128 = slice_zones(in_10, slot_4, oct_2, uv_7, d_7, _e94, _e95, band_in_1, _e116, soft, u);
                    rgb = _e128.ink.xyz;
            ...
            if (_e200) {
                ...
                NodeLayer _e230 = glyph_band(d_7, mark_in_1, mark_out_1, level_4, soft);
                NodeLayer _e231 = outer_glyph(slot_4, oct_2, uv_7, _e230, mark_in_1, mark_out_1, soft, u);
                float taper_1 = 1.0 - metal::smoothstep(1.5600001, QUAD_MARGIN, d_7);
                float _e237 = layer_coverage(_e231);
                float coverage_3 = _e237 * taper_1;
                ...
                float _e252 = mask_level(level_4);
                marks.mask = metal::max(_e249, (_e231.coverage * taper_1) * _e252);
                float _e257 = marks.sd;
                float _e262 = layer_distance(_e257, NodeLayer {_e231.sd * scale, level_4, _e231.coverage}, in_10);
                marks.sd = _e262;
            }
        }
    }
...
NodeInk node_ink(
    ...
    ink_1.lit = lit_2 / metal::max(_e90, 0.0001);
