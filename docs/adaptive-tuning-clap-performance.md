...
Untimed `params.flush` input stays untimed until a process boundary; its original offset and flush provenance remain visible.
...
A missing host output list produces unattempted completions; it never invents acceptance.
...
The revised contract subtotal is 130,029,056 bytes; with its existing 16 MiB allowance it remains below the 144 MiB session ceiling.
...
The output group allocation is 148,480 bytes plus a 1,280-byte fixed heap index in the owner; a completion is a bounded stack value whose reserved group cell remains owned during delivery.
...
The final focused boundary target passes 18 fixtures and the existing Harmonigraph configuration factory target passes all 21 fixtures.
No persisted configuration field, take format, latency setting or visual default changes here.

## Verification and remaining integration

The default vendored `clap_boundary` integration target exports combined, performance-only and legacy fixture classes through the real factory.
