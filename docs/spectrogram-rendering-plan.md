...
**Recommendation:** repair the boundedness and temporal defects first, take the tiny upload-allocation improvement, then measure a staged CPU handoff refactor.
...
| Component | Issue | Initial priority |
|---|---|---|
| SG1: bounded gap recovery | [#655](https://github.com/yan-h/harmonigraph/issues/655) | First correctness/resource repair |
| SG2: offline slice timestamps | [#656](https://github.com/yan-h/harmonigraph/issues/656) | Early, coordinated with replay work |
...
| `AudioSpectrum::push_samples` | Uses `ChannelBank` and an 8 ms sample-count hop, not the UI frame clock. Per-channel powers are summed; raw spectra become byte columns. The separate attack/release display filter serves the current curve, not history. Anchor smoothing maps sample counts to shell time; measurements are stamped half a window behind the newest sample. |
...
| Whole-song offline | `WholeSong::precompute` analyzes the requested window plus input margins once, with a span-dependent hop; columns and window remain fixed for that render. Each surface folds once, then redraws a static grid with a moving playhead. The frame loop also continues live analysis for the current spectrum curve. |
...
Reach requires no sample feeding during the gap, followed by resumed samples while old history remains inside the 610 s retention.
...
**Smallest useful change:** bound appends to the retained interval before filling a gap, discard unreachable prefix work, and release already pathological excess capacity at a defined reset/rebuild boundary.
...
An integration fixture must exercise `render.rs`'s actual sliced-audio caller at 30/60/120 fps, nonzero audio origin, late start, and a partial tail.
...
**Smallest useful change:** bound the sampling hop to an explicitly accepted coverage/attenuation policy and record its memory/analysis cost.
...
**Larger alternative:** analyze at the normal hop and MAX-accumulate directly into a bounded whole-song slab representation, retaining peak coverage without storing every raw column.
That preserves the separation between observation rate and display resolution at increased analysis time;
it should reuse the existing folding rules rather than introducing a second MAX implementation.

**Effort/maintenance:** medium for a bounded policy, medium–large for streaming preaggregation and its `WholeSong` representation.
...
The `ui/src/spectrum.rs` precompute-only slice is disjoint from the currently inspected tuning files;
an expansion into replay/take/configuration waits for their integration.
...
Bound retained data by the output slab budget plus explicit margins;
test trimmed windows and deterministic output.
...
**B, measurement-gated investment:** give the CPU slab store circular/keyed storage, identify changed tail/first/held slabs during folding, and hand over immutable deltas while retaining a coherent full snapshot or regeneration route for first upload, dropped callbacks, resize, and GPU eviction.
Start with reusable CPU snapshot/scratch storage if that captures enough benefit;
`Arc::make_mut` alone can still copy the entire run while the previous callback holds it.
Separate allocation reuse from removing full-run byte comparison so each improvement is measurable and reviewable.
...
otherwise stop after scratch reuse.
...
Coordinate with #642 rather than opening a second lattice timer project.
...
1. SG1 bounded gap recovery; SG2 timestamp repair as soon as the replay write set is integrated/coordinated.
2. SG3 whole-song coverage policy and phase-swept baseline, followed by the smallest accepted repair.
3. SG4 A can be an early independent small change; evaluate SG4 B after SG1 with stage measurements.
...
| SG3 precompute-only coverage repair | Can start in dedicated spectrum module; recheck test-file overlap | Independent until it changes shared frame ownership |
...
| SG6 egui timers/text/frame/export work | Recheck vendor/frames/spectral overlay paths | Coordinate #642 and shared text/shadow files; do not duplicate instrumentation |

The lattice plan merged as #652 and presently proposes, rather than implements, its components.
Its main renderer/lib and lattice shader changes are not automatic blockers for the spectrogram's dedicated modules.
...
Initial proposal, component briefs, source SHA, measurements, and reproduction packet are supplied identically to Claude Opus/xhigh (`fix=false`) and an independent read-only Astra reviewer.
Neither initial reviewer receives the other's report.
The final record will preserve their verdicts and independently validated dispositions;
revisions remain documentation only.
